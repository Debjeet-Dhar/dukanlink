  ...
  const stats = [
    {
      ...
      sub: products.length > 0 ? `Avg: ₹${averageProductPrice}` : "No products",
    },
    {
      label: "Total Products",
      value: String(products.length),
      sub: isPremium ? "Unlimited" : `${products.length}/${FREE_PRODUCT_LIMIT}`,
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img
            src={shop?.logo}
            alt={shop?.name}
            className="w-11 h-11 lg:w-12 lg:h-12 rounded-xl object-cover bg-primary-100"
            onError={(e) => {
              e.target.style.display = "none";
            }}
          />
          <div>
            <h1 className="text-lg lg:text-xl font-bold text-surface-900">
              My Shop Dashboard
            </h1>
      ...
      <div className="card mt-4 lg:mt-5">
        ...
        <div className="flex items-center gap-2 p-3 lg:p-4 bg-surface-50 rounded-xl border border-surface-200">
          <div className="flex-1 min-w-0">
            <p className="text-sm lg:text-base font-medium text-surface-700 truncate">
              {shopUrl}
            </p>
          </div>
        ...
        <Link
          ...
          target="_blank"
          className="inline-flex items-center gap-1.5 text-sm text-primary-600 font-medium mt-3 hover:text-primary-700 transition-colors"
        >
        ...
        {stats.map((stat) => (
          <div key={stat.label} className="card card-hover cursor-pointer">
            <div className="flex items-center gap-2 mb-3">
              <div
                className={`w-9 h-9 lg:w-10 lg:h-10 rounded-xl flex items-center justify-center ${stat.color}`}
              >
                {stat.icon}
              </div>
            </div>
            <p className="text-xs lg:text-sm text-surface-500 font-medium">
              {stat.label}
            </p>
            <p className="text-xl lg:text-2xl font-bold text-surface-900 mt-0.5">
              {stat.value}
            </p>
            {stat.sub && (
        ...
        {recentProducts.length === 0 ? (
          ...
          <div className="space-y-3">
            {recentProducts.map((product) => (
              <div
                key={product.id}
                className="flex items-center gap-3 p-2 lg:p-3 rounded-xl hover:bg-surface-50 transition-colors"
              >
                <img
                  src={product.image}
                  alt={product.name}
                  className="w-12 h-12 lg:w-14 lg:h-14 rounded-xl object-cover bg-surface-100"
                />
                <div className="flex-1 min-w-0">
                  <p className="text-sm lg:text-base font-semibold text-surface-900 truncate">
                    {product.name}
                  </p>
