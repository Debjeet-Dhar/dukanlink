...
export default function PublicShop() {
  ...
  const handleOrder = (productName) => {
    ...
    const productUrl = product
      ? `${window.location.origin}/shop/${shop.slug}/product/${product.id}`
      : "";
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <section className="pt-14 text-center sm:pt-16">
          <div className="mx-auto max-w-2xl">
            ...
            {shop.description && (
              <p className="mx-auto mt-2.5 max-w-2xl text-sm leading-6 text-surface-600 lg:text-base">
                {shop.description}
              </p>
            )}
        ...
        <div className="mb-5 mt-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold text-surface-900 lg:text-3xl">
              Featured Products
            </h2>
            <p className="mt-0.5 text-sm font-medium text-surface-500">
              Choose your item and order directly on WhatsApp.
            </p>
          </div>
          {categories.length > 1 && (
          ...
          <div className="grid grid-cols-1 gap-4 pb-10 min-[420px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:gap-6">
            {visibleProducts.map((product) => (
              <Link
                ...
                className="group flex flex-col overflow-hidden rounded-[24px] border border-surface-200/80 bg-white shadow-card transition-all duration-300 hover:-translate-y-1 hover:border-primary-200 hover:shadow-elevated"
              >
                <div className="relative m-2 overflow-hidden rounded-[20px] bg-gradient-to-br from-[#f3f5ef] to-[#f8faf5] shadow-inner">
                  <div className="flex h-56 items-center justify-center min-[420px]:h-44 sm:h-48 lg:h-52 relative">
                    <img
                      src={product.image}
                      alt={product.name}
                      className="h-full w-full object-contain p-4 transition-all duration-500 group-hover:scale-110 group-hover:rotate-1"
                    />
                  ...
                  {product.tags && product.tags.length > 0 && (
                    <div className="absolute left-3 top-3 z-10">
                      <span className="inline-block rounded-full bg-white/95 px-3 py-1.5 text-[11px] font-extrabold text-surface-800 shadow-soft backdrop-blur border border-white/20">
                        {product.tags[0]}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                    <div className="bg-white/90 backdrop-blur rounded-full p-2 shadow-soft">
                ...
                <div className="flex flex-col px-4 pb-4 pt-2">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      {product.category && (
                        <p className="mb-1 text-[11px] font-extrabold uppercase tracking-wide text-primary-700">
                          {product.category}
                        </p>
                      )}
                      <p className="line-clamp-2 text-base font-extrabold leading-5 text-surface-900">
                        {product.name}
                      </p>
                    </div>
                  </div>
                  {product.description && (
                    <p className="mt-1.5 line-clamp-2 text-xs leading-5 text-surface-500">
                      {product.description}
                    </p>
                  )}
                  <div className="pt-3">
                    <div className="mb-2.5 flex items-end justify-between gap-3">
                      <div>
                        <p className="text-[11px] font-bold uppercase tracking-wide text-surface-400">
                          Price
                        </p>
                    ...
                    <div className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-green-600 px-4 py-2.5 text-sm font-extrabold text-white shadow-soft transition-all hover:bg-green-700 hover:shadow-card active:scale-95">
