    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface-50">
        <div className="text-center">
          <p className="text-surface-400 font-medium text-lg">Shop not found</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-surface-50 page-enter">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="-mt-14 sm:-mt-18 lg:-mt-20 mb-4 flex items-end gap-4 lg:gap-5">
          ...
          <div className="pb-2">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-surface-900">
              {shop.name || "My Shop"}
            </h1>
        ...
        {shop.description && (
          <p className="text-surface-600 text-sm lg:text-base leading-relaxed mb-6 max-w-2xl">
            {shop.description}
          </p>
        )}
        ...
        <h2 className="text-lg lg:text-xl font-bold text-surface-900 mb-4">
          Products
        </h2>
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-4 pb-8">
            {products.map((product) => (
              <div
                key={product.id}
                className="bg-white rounded-2xl shadow-card overflow-hidden group hover:shadow-elevated hover:-translate-y-1 transition-all duration-300 flex flex-col"
              >
                <div className="relative overflow-hidden bg-surface-100">
                  <img
                    src={product.image}
                    alt={product.name}
                    className="w-full h-32 sm:h-36 lg:h-44 object-contain p-2 group-hover:scale-105 transition-transform duration-500"
                  />
